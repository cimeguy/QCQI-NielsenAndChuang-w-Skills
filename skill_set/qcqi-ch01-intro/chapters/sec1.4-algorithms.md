...
问题：$f:\{0,1\}\to\{0,1\}$，判断 $f$ 是**常数型**($f(0)=f(1)$) 还是**平衡型**($f(0)\ne f(1)$)。经典需 2 次求值。
1. 制备 $\ket{0}\ket{1}$，各过 $H$ 得 $(\ket{0}+\ket{1})(\ket{0}-\ket{1})/2$。
2. 作用一次 $U_f$：$\ket{x}\ket{y} \to \ket{x}\ket{y\oplus f(x)}$，相位回踢得到 $(-1)^{f(x)}$ 因子。
3. 对第一比特再过 $H$ 并测量：结果 $\ket{0}$ $\Rightarrow$ 常数型，$\ket{1}$ $\Rightarrow$ 平衡型。
- **结果**：每个线路只用了**一次** $U_f$，一次计算完成经典需两次才能完成的任务。
- **启发**：当经典计算获取的信息量**超过**问题答案所需信息量（即存在"计算冗余"）时，量子计算有潜在加速。Deutsch 问的是 $f(0)\oplus f(1)$ 这个全局比特，不需要分别知道 $f(0)$、$f(1)$。
...
2. Deutsch：一次 $U_f$ 判常数/平衡，揭示量子加速的本质直觉。
